...
public class DistanceDrive extends Command {
	...
	protected void execute() { 
		Logger.consoleLog("LeftSpeed: %s Right Speed: %s DistanceT: %s Ticks: %s", 
				speedL, speedR, gDistance(), Robot.driveBase.getLeftEncoder().getTicks());		
	...
	protected void end() {
		...
		Logger.consoleLog("LeftSpeed: %s Right Speed: %s DistanceT: %s Ticks: %s", 
				speedL, speedR, gDistance(), Robot.driveBase.getRightEncoder().getTicks());
	}
	...
	public void correct() {
		...
		if (Math.abs(currentTicksL - currentTicksR) < 3) {
			return;
	...
	// Make this return true when this Command no longer needs to run execute()
		...
		protected boolean isFinished() {
			return gDistance() > Math.abs(dist);
			//return (this.error <= 1);
		}
		...
		{
			double dee = (Robot.driveBase.getLeftEncoder().getDistance() + Robot.driveBase.getRightEncoder().getDistance())/2; // avg of both
			return Math.abs(dee);
		}
}
